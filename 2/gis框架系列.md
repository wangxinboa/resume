...
&emsp;先说衍生的项目，因为这个一开始是小组内原本就有的，一开始的项目都是以现有的 sdk 为基础进行开发的，我只是进行调用，在项目中使用，对其原理并不懂。到了后来进行了重新开发，才开始了解内部的运行机制。
...
&emsp;值得一提的是，根据 websocket 传过来的车辆数据，显示车辆的时候采用了防抖机制，即过几秒钟如果没有对应的数据传过来的话，就会自动消失，有的话就一直显示，推迟消失。
